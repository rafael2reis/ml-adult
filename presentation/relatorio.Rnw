...
Os arquivos com as bases de treino e de teste precisaram ser tratados antes da construção do modelo no framework Weka. Foi verificado que, em certos atributos categóricos, apareciam valores na base de teste que não existiam na base de treino. Isso empedia o carregamento do conjunto de teste para a validação no Weka. A solução encontrada foi a criação de um \emph{script} na linguagem R que convertia as bases em formato txt/csv para o formato \emph{arff}, utilizado pelo Weka.
...
Random Forest foi apresentado em \cite{breiman2001random}. O método consiste na geração de várias árvores de decisão e a classificação é dada pela resposta mais votada. A construção das árvores possui uma particularidade: para um conjunto de $p$ atributos, a cada particionamento, são considerados apenas $m$ atributos aleatórios possíveis, onde $m \approx \sqrt{p}$.
...
Para a tarefa de classificação de renda do Adult Data Set, os modelos baseados em Random Tree, Random Forest ou Random Committee obtiveram resultados excepcionais, bem acima dos já registrados na literatura.
